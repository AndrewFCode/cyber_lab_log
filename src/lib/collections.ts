...
export function entryHref(entry: WritingEntry) {
  return `${COLLECTION_PATH[entry.collection]}/${entry.id}`;
}
...
export function neighbors<T extends { id: string; data: { title: string } }>(
  ...
  return {
    prev: prevEntry ? { href: hrefFor(prevEntry), title: prevEntry.data.title } : undefined,
    next: nextEntry ? { href: hrefFor(nextEntry), title: nextEntry.data.title } : undefined,
  };
}
